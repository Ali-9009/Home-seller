    ...
    return (
        <div className="md:p-0 px-4">
            ...
            <header className="bg-[#FAFAFA] border border-[#DADADA] rounded-full px-4 lg:px-8 mx-auto mt-4 max-w-[1440px]">
                    ...
                    <nav className="hidden md:flex gap-6 text-center">
                        {LINKS.map((link) => (
                            <NavLink
                                key={link.name}
                                to={link.path}
                                className={({ isActive }) => `${baseLinkClass} ${isActive ? "after:w-full" : ""}`}
                            >
                                {link.name}
                            </NavLink>
                        ))}
                ...
                <div
                    id="mobile-menu"
                    className="fixed top-0 left-0 h-full w-64 bg-white shadow-lg transform -translate-x-full transition-transform duration-300 z-50 md:hidden"
                >
